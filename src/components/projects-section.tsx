    ...
    <SectionShell id="projects">
      ...
      <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2">
        {projects.map((project, index) => (
          <Reveal key={project.title} delay={(index % 2) * 100}>
            <Card className="group overflow-hidden border-border/60 transition-all duration-300 hover:-translate-y-1 hover:border-transparent hover:glow-ring">
              <button
                type="button"
      ...
      <Dialog open={openImage !== null} onOpenChange={(open) => !open && setOpenImage(null)}>
        <DialogContent className="sm:max-w-3xl">
          ...
          {openImage && (
            <img src={openImage} alt="Project screenshot" className="w-full rounded-md" />
          )}
        </DialogContent>
      </Dialog>
    </SectionShell>
